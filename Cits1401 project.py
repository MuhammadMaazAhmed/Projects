...
def main(textfile1,textfile2,feature):
    (w1,c1) = parse_txt(textfile1)
    (w2,c2) = parse_txt(textfile2)
    if feature=='conjunctions':
        profile_1=conjunctions(w1)
        profile_2=conjunctions(w2)
    if feature=='unigrams':
        profile_1=unigrams(w1)
        profile_2=unigrams(w2)
    if feature=='punctuations':
        profile_1=punctuations(w1,c1)
        profile_2=punctuations(w2,c2)
    if feature=='composite':
        profile_1=composite(w1,c1)
        profile_2=composite(w2,c2)
        
    dict1=dict(profile_1)
    dict2=dict(profile_2)
    d=distance(profile_1,profile_2)
...
def punctuations(w,c): # c=[numb of ';' , numb of ',', numb of sentences, numb of paragraphs]
    punctuation = {';':0,"'":0,'-':0,',':0}
    punctuation[';']=c[0] 
    punctuation[',']=c[1]
    for line in w:
...
def composite(w,c):
    ...
    numb_of_para=c[3]
    p1['words_per_sentence']= round(words/numb_of_sentences,4)
    p1['sentences_per_par']= round(numb_of_sentences/numb_of_para)
    return (p1)
